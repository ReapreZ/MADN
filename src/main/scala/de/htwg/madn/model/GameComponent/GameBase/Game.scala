...
case class Game(
    playerturn: Int,
    mesh: Mesh,
    piecesOutList: List[Int] = List(0, 0, 0, 0),
  ...
  def move(rolledDice: Int): Game = {
    if (rolledDice != 6) rolledDiceIsNotSix(rolledDice) else rolledDiceIsSix()
  }
  ...
  private def handleOnePieceOut(rolledDice: Int): Game = {
    updatePlayerTurn(movePiece(rolledDice, 1))
  }
  def updatePlayerTurn(game: Game): Game = {
    if (playerturn == mesh.Player) game.copy(playerturn = 1)
    else game.copy(playerturn = increment(playerturn))
  }
  private def handleNoPiecesOut(): Game = {
    if (playerturn == mesh.Player) resetPlayerTurn() else incrementPlayerTurn()
  }
  private def resetPlayerTurn(): Game =
    copy(playerturn = 1, timesPlayerRolledList = resetTimesPlayerRolled())
  private def incrementPlayerTurn(): Game = copy(
    playerturn = increment(playerturn),
    timesPlayerRolledList = resetTimesPlayerRolled()
  )
  private def resetTimesPlayerRolled(): List[Int] = {
    timesPlayerRolledList.updated(decrement(playerturn), 0)
  }
  ...
  private def updateField(
    ...
    getTurnC(playerturn) match {
      case Success(v) =>
        game.mesh.field1.Arr(
          (mesh.piecepos(currentPlayer)(currentPiece)) + rolledDice
        ) = v
      case Failure(e) => println(e.getMessage)
    }
  }
  def piecesOutLessThanFour(
      player: Int,
      nextPlayerField: Int,
  ...
  def isFieldOccupied(rolledDice: Int, piece: Int): Game = {
    checkForPlayer(0, 0, determineNewPos(rolledDice)(piece))
  }
  ...
  def checkForField(counter: Int, counter2: Int, newPos: Int): Game = {
    if (counter2 != 4) {
      if (
        mesh.piecepos(counter)(counter2) == newPos && decrement(
          playerturn
        ) != counter
      ) {
  ...
  def determineNewPos(rolledDice: Int): Int => Int = (piece: Int) => {
    mesh.piecepos(decrement(playerturn))(decrement(piece)) + rolledDice
  }
  def setHouseInMesh(
      houseChar: Char,
      housePosition: Int,
    ...
    mesh.house1.Arr(decrement(piecesOutList(piecePosition)) + housePosition) =
      houseChar
  }
  ...
  def startgame: Try[Game] = {
    ...
    for {
      ...
      _ <- Try(
        require(
          input >= 1 && input <= 4,
          "Invalid number of players: must be between 1 and 4."
        )
      )
  ...
  def rollAgain(): Game = {
    copy(timesPlayerRolledList =
      changeList(timesPlayerRolledList, decrement(playerturn), 1)
    )
  }
  def moveChosenPiece(chosenPiece: Int): Game = {
    if (piecesOutList(decrement(playerturn)) == chosenPiece)
      movePiece(6, chosenPiece)
    else movePieceOut()
  }
